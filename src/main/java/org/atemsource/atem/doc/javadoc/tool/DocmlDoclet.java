...
/*
 * File: JELDoclet.java Purpose: A Doclet to be used with JavaDoc which will output XML with all of the information from
 * the JavaDoc. Date: Mar 2nd, 2003 History: Sep 14th, 2005 - Updated by TP to allow multiple file output. - Added
 * support for exceptions. - Added support for a few missing method modifiers (final, etc). - Added support for xml
 * namespaces. Dec 8/9th, 2005 - updated by T.Zwolsky (all extensions marked thz.../thz): - added cmdline parameter
 * -outputEncoding - bugfix: implements/interface node(s) were created but not inserted into class node - added
 * exception comment (both here and in the xsd as optional element) - added cmdline parameter -filename - added output
 * directory check - added some comments to readme - added test target in build.xml - added nested class in test classes
 * (test/MyInterClass.java) - added xsl transformation jel2html.xsl Dec 16th, 2005 - updated by T.Zwolsky (all
 * extensions marked thz.../thz): - added version here and in xsd - added admin stuff in xsd Author: Jack D. Herrington
 * <jack_d_herrington@codegeneration.net> Toby Patke <toby_patke _?_ hotmail.com> This source is covered by the Open
