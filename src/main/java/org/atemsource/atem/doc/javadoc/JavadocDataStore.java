...
public class JavadocDataStore implements MetaDataService, EntityTypeRepositoryPostProcessor, MetaAttributeService
{
	...
	private final Map<String, ClassDescription> entityTypeData = new Hashtable<String, ClassDescription>();
	...
	private ClassDescription convert(ClassDescription classDescription)
	{
	...
	public ClassDescription getDescription(Class<?> clazz)
	{
		...
		{
			return convert(entityTypeData.get(clazz.getName()));
		}
		else
		{
			load(clazz.getName());
			ClassDescription classDescription = convert(entityTypeData.get(clazz.getName()));
	...
	public Object getMetaData(Object targetEntity, MetaAttribute attribute)
	{
		...
		if (targetEntity instanceof Attribute)
		{
			...
			ClassDescription value = convert(entityTypeData.get(targetAttribute.getEntityType().getCode()));
			if (value == null && !entityTypeData.containsKey(targetAttribute.getEntityType().getCode()))
			{
				load(targetAttribute.getEntityType());
				value = convert(entityTypeData.get(targetAttribute.getEntityType().getCode()));
				if (value == null)
				{
					return null;
				}
		...
		else if (targetEntity instanceof EntityType)
		{
			...
			ClassDescription value = convert(entityTypeData.get(targetType.getCode()));
			if (value == null)
			{
				load(targetType);
				return convert(entityTypeData.get(targetType.getCode()));
			}
			else
			{
				return value;
	...
	{
		URL resource = getClass().getResource("/" + name.replace(".", "/") + ".docml");
		if (resource == null)
		{
			entityTypeData.put(name, new ClassDescription());
			return;
		}
		...
		if (file.exists())
		{
			...
			try
			{
				...
				entityTypeData.put(name, object);
			}
			catch (JAXBException e)
			{
				entityTypeData.put(name, new ClassDescription());
